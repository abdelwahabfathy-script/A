...
export default function MobileFrame({ children, themeMode, language, isPwa = false }: MobileFrameProps) {
  ...
  return (
    <div className={`min-h-screen w-full flex items-center justify-center transition-colors duration-300 ${
      ...
      >
        {/* Physical Camera Notch (hidden on mobile and in PWA mode, simulated on desktop browser) */}
        {!isPwa && (
        ...
        {/* Dynamic Android Status Bar (completly hidden in true full-screen PWA mode to maximize writing space) */}
        {!isPwa && (
        ...
        {/* Android Digital Navigation Pill Mockup (omitted in native standalone mode for maximum writing space) */}
        {!isPwa && (
